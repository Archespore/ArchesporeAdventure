...
public class FieryFisherAbility extends SkillAbility {

	public FieryFisherAbility(List<String> displayStrings, Material material, int minLevel, double minChance,
	...
	public boolean abilityEffect(Event event) {
		if (event instanceof PlayerFishEvent) {
			...
			if (ThreadLocalRandom.current().nextDouble(100) < getChanceForPlayer(player) && fishedEntity instanceof Item) {
				...
				if (fishedItemType.equals(Material.COD) || fishedItemType.equals(Material.SALMON)) {
					fishedItem.getItemStack().setType(Material.valueOf("COOKED_" + fishedItemType.toString()));
				}
			}
		}
		return false;
