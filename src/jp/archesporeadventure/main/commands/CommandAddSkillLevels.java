...
public class CommandAddSkillLevels implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (args.length >= 3) {
			...
			if (giftedPlayer != null && playerSkillController.doesPlayerExist(giftedPlayer) && playerSkillController.getSkillNameSet().contains(args[1].toUpperCase())) {
				...
				playerSkillController.addPlayerLevel(giftedPlayer, skill, amount);
				sender.sendMessage(ChatColor.GREEN + "Gave " + amount + " skill levels to the player, " + giftedPlayer.getName());
				return true;
			}
		}
